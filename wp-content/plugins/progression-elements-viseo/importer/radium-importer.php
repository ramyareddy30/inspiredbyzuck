 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
	    ...
	    public function __construct() {
		   ...
		   $this->slider 			= apply_filters('radium_theme_importer_slider_file', $this->slider_file_name);
			...
			?>
			 ...
			 if( !empty($this->imported_demos) ) { ?>
			  	...
			  	<div style="background-color: #FAFFFB; margin:10px 0;padding: 5px 10px;color: #8AB38A;border: 2px solid #a1d3a2; clear:both; width:90%; line-height:18px;">
			  		<p><?php _e('Demo already imported', 'progression-elements-viseo'); ?></p>
			  	</div>
	    ...
	    public function demo_installer() {
			...
			$coloroption = isset($_POST['coloroption']) ? $_POST['coloroption'] : '';
			$slideroption = isset($_POST['slideroption']) ? $_POST['slideroption'] : '';
			
			

			if( !empty($this->imported_demos ) ) {

				$button_text = esc_html__('Import Again', 'progression-elements-viseo');

			} else {

				$button_text = esc_html__('Import Demo Data', 'progression-elements-viseo');

			}

	        ?>
            <div id="icon-tools" class="icon32"><br></div>
	        <h2><?php echo esc_html__('Import Demo Data', 'progression-elements-viseo'); ?></h2>
   			...
   			<div style="background-color: #F5FAFD; margin:10px 0;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:90%; line-height:18px;">
   			    <p class="tie_message_hint"><?php echo esc_html__('Tips', 'progression-elements-viseo'); ?></p>

   			      <ul style="padding-left: 20px;list-style-position: inside;list-style-type: square;}">
 						 <li><?php echo esc_html__('The importer will not delete any pages, posts, or content.  It will only add content and adjust theme options.', 'progression-elements-viseo'); ?></li>
   						 <li><?php echo esc_html__('If the import times out or does not finish, please run the importer again.', 'progression-elements-viseo'); ?> </li>		
 						 <li><?php echo esc_html__('Running the importer mulitple times may duplicate your menu items. If you run the importer again, make sure to delete any navigation menus under Navigation > Menus.  You will not have delete any pages as those will not duplicate.', 'progression-elements-viseo'); ?></li>
   			      </ul>
   			 </div>
	       	...
	       	<div class="radium-importer-wrap" data-demo-id="1"  data-nonce="<?php echo wp_create_nonce('radium-demo-code'); ?>">
				...
				foreach($name_of_thmems as $key => $name123)
				{
					
					$plugin_url=plugin_dir_url( __DIR__ ).'demo-files/'.$name123.'/screenshot.png';
					?>
                    <form name="import_form_<?php echo $key; ?>" method="post">
                        <div class="importoptions" style="border: 1px solid #ccc;padding: 1%;width: 24%; float:left; margin:1%;">
                           ...
                           <h1 style="text-align:center;"><?php echo $name123?></h1>
                           ...
                           <!--<input type="checkbox" name="coloroption" value="coloroption" id="coloroption">Color Options</input>-->
                           <!--div class="pro-input">
                               <input type="checkbox" name="slideroption" value="slideroption" id="slideroption">Slider</input>
                           </div-->
                           <div class="pro-input-button">			
                           <input name="reset" class="panel-save button-primary radium-import-start" type="submit" value="<?php echo $button_text ; ?>" />
                           <input type="hidden" name="theme_name_hid" value="<?php echo $name123;?>" />
                           <input type="hidden" name="action" value="demo-import" id="impor_demo_id" />
                            ...
                            if($theme_name_hid==$name123){
                                if( $action == 'demo-import' && check_admin_referer('radium-demo-code' , 'demononce')){
                                    $path11=$this->demo_files_path.$theme_name_hid.'/';     
                                    $return_value=$this->process_imports($democontent,$widgetsoption,$themeoption,$coloroption,$slideroption,$path11);
                                }
                            }
                            ?>
                            </div>
	     ...
	     * @return null
	     */
	    public function process_imports($democontent,$widgetsoption,$themeoption,$coloroption,$slideroption,$path11) {
			...
			if($democontent != ''){
				...
				if (!empty( $democontentpath ) && is_file($democontentpath ) ) {
					//$this->set_demo_data( $democontentpath );
					 $this->set_demo_data( $path11.$this->content_demo );
				}
				
				$this->set_demo_menus();
				
				//if ( is_file( main_menu ) ) {
					...
					$blogpage = get_page_by_title ('Latest Posts');
			...
			if($slideroption != '')
			{
				//$slideroptionpath=$path11.$this->slider_file_name;
					$dh= opendir($path11);
					...
					foreach($files as $file_name)
					{
						...
						if($ext == 'zip'){
							$slideroptionpath=$path11.$file_name;
							if ( class_exists( 'RevSlider' ) )
							{
									if ( file_exists( $slideroptionpath) ) {	
									$newslider = new RevSlider();
									$msd= $newslider->importSliderFromPost( true, true, $slideroptionpath);
								//	print_r($msd);
									if($msd['success'] == 1)
									{
										echo "<h3>".$file_name." Slider has Imported<h3>";
			...
			if($widgetsoption != ''){  
				 ...
				 $widgetsoptionpath=$path11.$this->widgets_file_name;
				if (  !empty( $widgetsoptionpath ) && is_file( $widgetsoptionpath ) ) {
					...
					echo "<h3 style='color:RED'>widgets Import File canot Exist <h3>";
					}
			}
			if($themeoption != ''){  	
				$themeoptionpath=$path11."theme_option.dat";
		...
		if ( ! file_exists( $dir ) ) {
			$cei_error = esc_html__( 'Error importing settings! Please try again.', 'progression-elements-viseo' );
			return;
		}
		...
		if ( 'array' != gettype( $data ) ) {
			...
			$cei_error = esc_html__( 'Error importing settings! Please check that you uploaded a customizer export file.', 'progression-elements-viseo' );
			return;
		}
		...
		if ( $data['template'] != $template ) {
			...
			//$cei_error = esc_html__( 'Error importing settings! The settings you uploaded are not for the current theme.', 'progression-elements-viseo' );
			//return;
		}
	...
	static public function _is_image_url( $string = '' ) 
	    ...
	    public function set_demo_menus() {
			...
			if ( isset( $main_menu->term_id ) ) {
				set_theme_mod( 'nav_menu_locations', array(
						'progression-studios-primary' => $main_menu->term_id,
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			esc_html__( 'Widget Import file could not be found. Please try again.', 'progression-elements-viseo' ),
	    			'',
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'progression-elements-viseo' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = esc_html__( 'Site does not support widget', 'progression-elements-viseo' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = esc_html__( 'Widget already exists', 'progression-elements-viseo' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = esc_html__( 'Imported', 'progression-elements-viseo' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = esc_html__( 'Imported to Inactive', 'progression-elements-viseo' );
	    				}
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'progression-elements-viseo' ); // show "No Title" if widget instance is untitled
